...
parser.add_argument('--resnet_type', default='resnet50')
...
with open(os.path.join(args.log_dir, 'args_{0}_{1}.yml'.format(args.seed, args.sparcity)), 'w') as f:
...
logger = Logger('logs_{0}_{1}'.format(
    args.seed, args.sparcity), base=args.log_dir, fmt=fmt)
...
torch.save(model.state_dict(), os.path.join(
    args.log_dir, 'model.torch_{0}_{1}'.format(args.seed, args.sparcity)))
...
for e in range(1, args.epochs + 1):
    ...
    with torch.no_grad():
        pred = torch.cat(list(
            map(lambda x: model(x[0].to(device)).argmax(dim=1), testloader))).cpu().numpy()
