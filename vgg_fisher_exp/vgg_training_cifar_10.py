...
testset = torchvision.datasets.CIFAR10(root='./data', train=False,
                                       download=True, transform=transform)
data_test_loader = torch.utils.data.DataLoader(testset, batch_size=256,
...
def prune_and_learn(net, tresh, last=None):
...
for i in range(n):
    ...
    torch.save(vgg.state_dict(), 'vgg_cifar10_'+str(i)+'.pth')
    for j, tresh in enumerate(tresholds):
        accuracy_after_pruning = prune_and_learn(copy.deepcopy(vgg), tresh, last=1 if tresh==90 else None)
